...
export const Navigation = () => {
  ...
  return (
    <>
      <aside
        ref = {sidebarRef}
        className={cn('group/sidebar h-full bg-secondary overflow-y-auto relative w-60 flex-col z-[9999]',
      ...
      <div
        ref={navbarRef}
        className={cn("absolute top-0 z-[99999] left-60 w-[calc(100%-240px)]",isResetting && "translate-all ease-in-out duration-300", isMobile && "left-0 w-full")}
      >
        <nav className="bg-transparent px-3 py-2 w-full">
          {isCollapsed && <MenuIcon role = "button" className="h-6 w-4 text-muted-foreground"/>}
        </nav>
      </div>
    </>
  )
