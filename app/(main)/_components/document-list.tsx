...
export const DocumentList = ({
  ...
  const [expanded, setExpanded] = useState<Record<string,boolean>>({}) // {documentId:string : isExpanded:boolean}
  const onExpand = (documentId: string) => {
    setExpanded(preExpanded => ({
      ...preExpanded,
      [documentId]: !preExpanded[documentId]
    }))
    console.log('expanded', expanded, level)
  }
    ...
    <>
      ...
      {documents.map((document)=>(
        <div key={document._id}>
          ...
          />
          {expanded[document._id] && (<DocumentList parentDocumentId={document._id}
          level={level + 1}
          />)}
        </div>
      ))}
